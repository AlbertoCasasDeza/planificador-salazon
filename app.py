...
replan_mode = st.sidebar.selectbox(
    "Modo al aplicar planificación",
    ["Solo rellenar vacíos", "Replanificar días con override", "Replanificar todo"],
    index=0
)
...
def planificar_filas_na(
    ...
    # Helpers: fallback para overrides CAP1/CAP2
    ...
    # Helpers: capacidad por día/intent separadas para ENTRADA y SALIDA (con fallback)
    def get_cap_ent(date_dt, attempt):
        dkey = pd.to_datetime(date_dt).normalize()
        ov = cap_overrides_ent.get(dkey)
        if ov is not None:
            cap_try = ov.get("CAP1") if attempt == 1 else ov.get("CAP2")
            cap = _first_not_none(cap_try, ov.get("CAP1"), ov.get("CAP2"))
            if cap is not None:
                return cap
    ...
    # ===============================
    # ASIGNACIÓN de ENTRADA con prioridades:
    # (1) objetivo/balance de carga (con override si existe),
    # (2) minimizar cambios de TIPO NITRIF,
    # (3) minimizar cambios de NITRIF,
    # (4) fecha más temprana.
    # ===============================
    ...
    # Helpers para objetivos (override) del día/attempt
    def _target_cap_ent(d, attempt):
        ...
        # fallback al otro si falta
        if val is None or pd.isna(val):
            val = ov.get("CAP2") if attempt == 1 else ov.get("CAP1")
    ...
    for idx, row in pendientes.iterrows():
        ...
        asignado = False
        for attempt in [1, 2]:
            raw_candidatos = []  # (ent, sal, cap_e, load_e, cap_s, load_s, cost_tipo, cost_nitr, tgt_e, tgt_s)
            entrada = entrada_ini
            ...
            if raw_candidatos:
                ...
                for (ent, sal, cap_e, load_e, cap_s, load_s, cost_tipo, cost_nitr, tgt_e, tgt_s) in raw_candidatos:
                    ...
                    # 🔑 Prioridad: balance/objetivo -> tipo -> nitrif -> fecha
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Aplicar planificación"):
        # Prepara df de entrada según modo elegido
        df_input = df.copy()

        if replan_mode == "Replanificar días con override":
            fechas_afectadas = set(cap_overrides_ent.keys()) | set(cap_overrides_sal.keys())
            if fechas_afectadas and {"ENTRADA_SAL", "SALIDA_SAL"}.issubset(df_input.columns):
                mask = df_input["ENTRADA_SAL"].notna() & df_input["ENTRADA_SAL"].dt.normalize().isin(fechas_afectadas)
                df_input.loc[mask, ["ENTRADA_SAL", "SALIDA_SAL"]] = pd.NaT

        elif replan_mode == "Replanificar todo":
            if {"ENTRADA_SAL", "SALIDA_SAL"}.issubset(df_input.columns):
                df_input[["ENTRADA_SAL", "SALIDA_SAL"]] = pd.NaT

        df_planificado = planificar_filas_na(
            df_input, dias_max_almacen_global, dias_max_por_producto,
            estab_cap, cap_overrides_ent, cap_overrides_sal, estab_cap_overrides
        )
        st.session_state["df_planificado"] = df_planificado
        st.success("✅ Planificación aplicada.")
    ...
    if "df_planificado" in st.session_state:
        ...
        # -------------------------------
        # Gráfico: Entradas y salidas por fecha con detalle por lote
        # -------------------------------
